...
try:
    from pyspark.sql.utils import require_minimum_pyarrow_version, require_maximum_pyarrow_version
    require_minimum_pyarrow_version()
    require_maximum_pyarrow_version()
except ImportError as e:
...
class SQLTests(ReusedSQLTestCase):
    ...
    def test_udf_in_subquery(self):
        ...
        self.spark.range(1).filter(f("id") >= 0).createOrReplaceTempView("v")
        sql = self.spark.sql
...
class QueryExecutionListenerTests(unittest.TestCase, SQLTestUtils):
    ...
    def test_execution_time(self):
        ...
        def sleep_df(num_seconds):
            def sleep_fcn(num_seconds):
                time.sleep(num_seconds)
                return num_seconds
            sleep_udf = udf(lambda num_seconds: sleep_fcn(num_seconds), FloatType())
            return self.spark.createDataFrame(
                data=self.spark.sparkContext.parallelize([1.0] * num_seconds, numSlices=1),
                schema=FloatType()).select(sleep_udf(col("value")))

        def check_within_tolerance(value, expected, tolerance_pcnt):
            return 1.0 - tolerance_pcnt <= float(value) / float(expected) <= 1.0 + tolerance_pcnt

        test_duration_sec = 10  # the higher the lower the chance of flakiness
        tolerance_pcnt = 0.25   # same here

        # prepare
        self.spark.conf.set("spark.databricks.execution.collectSpillThreshold", 1)
        test_df = sleep_df(test_duration_sec)

        # warm up
        self.spark.range(10).collect()

        # reset counters
        self.spark._jvm.OnSuccessCall.clear()
        self.assertTrue(not self.spark._jvm.OnSuccessCall.isCalled())
        ...
        reported_duration_sec = self.spark._jvm.OnSuccessCall.getLastDurationNs() / 1e9
        self.assertTrue(
            check_within_tolerance(actual_duration_sec, test_duration_sec, tolerance_pcnt) and
            check_within_tolerance(reported_duration_sec, test_duration_sec, tolerance_pcnt),
            "\n\ttest_job_duration = {} \n\tactual_job_duration = {} \n\treported_job_duration = {}"
            .format(test_duration_sec, actual_duration_sec, reported_duration_sec))
...
class HiveContextSQLTests(ReusedPySparkTestCase):
    ...
    def test_limit_and_take(self):
        ...
        def assert_runs_only_one_job_stage_and_task(job_group_name, f):
            ...
            def assert_one_job_stage_and_task():
...
class TrustedFilePathTests(ReusedSQLTestCase):
    @classmethod
    def conf(cls):
        return super(TrustedFilePathTests, cls).conf()\
...
class ScalarPandasUDFTests(ReusedSQLTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.spark.conf.set("spark.databricks.execution.pandasUDF.prefetch.enabled", False)
    ...
    def test_vectorized_udf_basic(self):
        from pyspark.sql.functions import pandas_udf, PandasUDFType, col, array
    ...
    def test_vectorized_udf_struct_type(self):
        from distutils.version import LooseVersion
        from pyspark.sql.functions import pandas_udf, PandasUDFType, col, struct
        import pandas as pd
        import pyarrow as pa
    ...
    def test_nondeterministic_vectorized_udf(self):
        # Test that nondeterministic UDFs are evaluated only once in chained UDF evaluations
        from pyspark.sql.functions import udf, pandas_udf, col, PandasUDFType
...
class GroupedMapPandasUDFTests(ReusedSQLTestCase):
    ...
    def test_supported_types(self):
        ...
        # Tests below use pd.DataFrame.assign that will infer mixed types (unicode/str) for column
        # names from kwargs w/ Python 2, so need to set check_column_type=False and avoid this check
        if sys.version < '3':
