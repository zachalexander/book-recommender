...
def require_maximum_pyarrow_version():
    """ Raise ImportError if maximum version of pyarrow is not installed
    """
    maximum_pyarrow_version = "0.11.1"
    ...
    if not have_arrow:
        raise ImportError("PyArrow <= %s must be installed; however, "
                          "it was not found." % minimum_pyarrow_version)
    if LooseVersion(pyarrow.__version__) > LooseVersion(maximum_pyarrow_version):
        raise ImportError("PyArrow <= %s must be installed; however, "
                          "your version was %s." % (maximum_pyarrow_version, pyarrow.__version__))
