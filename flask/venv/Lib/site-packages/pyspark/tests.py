...
class SparkClientContextTests(PySparkTestCase):

    def setUp(self):
        self.original_system_fn = platform.system
        super(SparkClientContextTests, self).setUp()
        platform.system = lambda: "Windows"

    def tearDown(self):
        platform.system = self.original_system_fn
        super(SparkClientContextTests, self).tearDown()

    def test_normalize_windows_path(self):
        """Test normalization of windows path"""

        self.sc._conf.set("spark.databricks.service.client.enabled", "true")
        self.sc._conf \
            .set("spark.databricks.service.client.windows.path.normalizationEnabled", "true")
        SparkSession.builder.getOrCreate()
        test_file_name = "Users\\testfolrder\\testfile"
        os.path.sep = "\\"
        rdd = self.sc.textFile(test_file_name)
        self.assertEqual(rdd.name(), test_file_name.replace("\\", "/"))
        rdd = self.sc.wholeTextFiles(test_file_name)
        self.assertEqual(rdd.name(), test_file_name.replace("\\", "/"))
        rdd = self.sc.binaryFiles(test_file_name)
        self.assertEqual(rdd.name(), test_file_name.replace("\\", "/"))
...
class ZZZ_ContextLocalFileSystemTests(unittest.TestCase):
    def get_spark_session(self):
        conf = SparkConf().set(
            ...
            "spark.databricks.pyspark.onlyAllowTrustedFilesystems", "true").set(
            # Use trusted data transfer to make sc.parallelize work
            "spark.databricks.pyspark.trustedJVMDataTransfer.enabled", "true").set(
            # Enforce that the local filesystem is used by default when no scheme is specified
            "spark.hadoop.fs.default.name", "file:///").set(
            # Enforce that we actually use LocalFileSystem for the local filesystem
            "spark.hadoop.fs.file.impl", "org.apache.hadoop.fs.LocalFileSystem").set(
            "spark.hadoop.fs.default.impl", "org.apache.hadoop.fs.LocalFileSystem")
        sc = SparkContext(conf=conf)
    ...
    def test_new_api_hadoop_file(self):
        self.local_file_method_test(lambda sc, path: sc.newAPIHadoopFile(
            path,
            "org.apache.hadoop.mapreduce.lib.input.SequenceFileInputFormat",
            "org.apache.hadoop.io.IntWritable",
            "org.apache.hadoop.io.Text"))
    ...
    def test_hadoop_file(self):
        self.local_file_method_test(lambda sc, path: sc.hadoopFile(
            path,
            "org.apache.hadoop.mapred.SequenceFileInputFormat",
            "org.apache.hadoop.io.IntWritable",
            "org.apache.hadoop.io.Text"))
...
class ZZZ_RDDLocalFileSystemTests(unittest.TestCase):
    ...
    def test_save_as_text_file(self):
        self.local_file_method_test(
            lambda sc, path: sc.parallelize(range(10)).saveAsTextFile(path))
...
class ZZZ_FileBasedCollectTests(unittest.TestCase):
    def get_spark_session(self):
        conf = SparkConf().set(
            ...
            "spark.databricks.pyspark.useFileBasedCollect", "true").set(
            # Enforce that the local filesystem is used by default when no scheme is specified
            "spark.hadoop.fs.default.name", "file:///").set(
            # Enforce that we actually use LocalFileSystem for the local filesystem
            "spark.hadoop.fs.file.impl", "org.apache.hadoop.fs.LocalFileSystem").set(
